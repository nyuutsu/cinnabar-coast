...
promoteTransient raw = InterpretedTransient
  { transLetterDelay        = TransientByte (fromIntegral (rawLetterDelay raw))
  , transMusicId            = TransientByte (fromIntegral (rawMusicId raw))
  , transMusicBank          = TransientByte (fromIntegral (rawMusicBank raw))
  , transContrastId         = TransientByte (fromIntegral (rawContrastId raw))
  , transEnemyTrainerClass  = TransientByte (fromIntegral (rawEnemyTrainerClass raw))
  , transBoulderSpriteIndex = TransientByte (fromIntegral (rawBoulderSpriteIndex raw))
  , transDungeonWarpDest    = TransientByte (fromIntegral (rawDungeonWarpDest raw))
  , transDungeonWarpUsed    = TransientByte (fromIntegral (rawDungeonWarpUsed raw))
  , transWarpedFromWarp     = TransientByte (fromIntegral (rawWarpedFromWarp raw))
  , transWarpedFromMap      = TransientByte (fromIntegral (rawWarpedFromMap raw))
  , transCardKeyDoorY       = TransientByte (fromIntegral (rawCardKeyDoorY raw))
  , transCardKeyDoorX       = TransientByte (fromIntegral (rawCardKeyDoorX raw))
  , transTrashCanLock1      = TransientByte (fromIntegral (rawTrashCanLock1 raw))
  , transTrashCanLock2      = TransientByte (fromIntegral (rawTrashCanLock2 raw))
  , transCurrentMapScript   = TransientByte (fromIntegral (rawCurrentMapScript raw))
  }
